...
class QuestionsFrame(tk.Frame):
    ...
    def show_next_question(self):
        if not self.additional_questions_displayed:
            if (
                (self.current_question == 1 and len(self.answers) >= 2 and self.answers[1] == "Yes")
                or (self.current_question == 2 and len(self.answers) >= 3 and self.answers[2] == "Yes")
                or (self.current_question == 3 and len(self.answers) >= 4 and self.answers[3] == "Yes")
                or (self.current_question == 4 and len(self.answers) >= 5 and any(answer.startswith("Yes") for answer in self.answers[4:]))
            ):
